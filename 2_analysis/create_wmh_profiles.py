...
def collate_wmh_profiles(csv_list, output_bname, chunk_dir, participant):
  ...
  if curr_list: # if files found
    ...
    for fname in curr_list: # for each participant file
      ...
      method = op.basename(op.dirname(fname)) # extract method string
...
def main(csv_list, output_fname, max_workers = 10):
  ...
  # prepare collate_wmh_profiles function with partial arguments
  thread_func = partial(collate_wmh_profiles, csv_list, output_bname, chunk_dir)
...
if __name__ == "__main__":
  ...
  profiles_pattern = op.join("profiles", "sub-*", "afq-*", "*_mask.csv")
  profiles_list = [
    *glob.glob(op.join(paths_multi,  profiles_pattern)),  
    *glob.glob(op.join(paths_single, profiles_pattern))
  ]

  main(profiles_list, op.join(output_dir, "wmh_profiles.csv"))